        ...
        for lat, lon in locations:
            try:
                if use_observations and station_map and (lat, lon) in station_map:
                    ...
                    df = self.fetch_observations_for_station(station_id, start_date, end_date)
                else:
                    df = self.fetch_forecast_for_location(lat, lon, start_date, end_date)
...
def save_weather_bronze(
    ...
    metadata = {
        ...
        'date_range': {
            'start': weather_df['timestamp'].min().isoformat() if 'timestamp' in weather_df.columns else None,
            'end': weather_df['timestamp'].max().isoformat() if 'timestamp' in weather_df.columns else None
        },
        'locations': weather_df[['lat', 'lon']].drop_duplicates().to_dict('records') if 'lat' in weather_df.columns else []
    }
...
def fetch_and_save_weather(
    ...
    logger.info(f"Fetching weather data for {len(locations)} locations from {start_date} to {end_date}")
