...
def test_without_incident_count_t(
    ...
    if len(columns_to_remove) > 0:
        logger.info(
            f"  Removing {len(columns_to_remove)} columns related to incident_count:"
        )
        ...
        logger.info(
            "  ✓ No incident_count features found in X (good - no target leakage)"
        )
        logger.info(
            "  Testing with all features (weather, calendar, spatial, rolling stats)"
        )
    ...
    logger.info(
        "  Features kept: weather, calendar, spatial, rolling stats (non-incident)"
    )
...
def test_horizon_sensitivity(
    ...
    for horizon in horizons:
        ...
        y_target_sorted[f"incident_count_t_plus_{horizon}"] = y_target_sorted.groupby(
            "h3_cell"
        )["incident_count_t_plus_1"].shift(-(horizon - 1))

        # Filter to valid targets (same mask as base)
        valid_mask = valid_base & ~pd.isna(
            y_target_sorted[f"incident_count_t_plus_{horizon}"]
        )
        y_horizon = y_target_sorted[f"incident_count_t_plus_{horizon}"][
            valid_mask
        ].values
...
def test_sector_cold_start(
    ...
    logger.info(
        f"  Test cells (held out): {len(test_cells)} ({test_cell_fraction*100:.0f}%)"
    )
